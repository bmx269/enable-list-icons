# Enable List Icons - Development Guide

## Project Overview
WordPress plugin that adds icon support to List Block items (list-item). Uses `@wordpress/scripts` for build tooling.

## Repository Structure
```
├── enable-list-icons.php         # Main plugin file (PHP server-side logic)
...
- All user-facing strings must use `__()` / `_e()` with text domain `enable-list-icons`
...
- Version must be synced in: `package.json`, `enable-list-icons.php`, and `readme.txt`

## Key Architecture
- Icons are rendered server-side via `render_block` filters (not saved in block markup)
- Parent List block stores default icon settings; child items inherit or override
- List block defaults use a global stack (`$enable_list_icons_list_stack`) to handle nested list blocks
